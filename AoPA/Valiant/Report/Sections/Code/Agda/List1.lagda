...
data ℕ  : Set where
  ...
  suc   : ℕ → ℕ
\end{code}
This datatype has two constructors, |zero| and |suc|. The constructor  |zero| is an element of |ℕ|, intended to be interpreted as $0$, and the constructor |suc| is a function taking a natural number as input and returning what is to be thought of as the successor of the number. For example, we can define
\begin{code}
one : ℕ
one = suc zero
...
\end{code}
This is a fairly cumbersome way of writing numbers, and it is possible to make Agda support a more standard notation for elements of |ℕ| using the pragmas
\begin{code}
{-# BUILTIN NATURAL   ℕ      #-}
{-# BUILTIN ZERO      zero   #-}
{-# BUILTIN SUC       suc    #-}
...
infixr 8 _∷_                   
\end{code}
We have chosen the notation to be similar to the Haskell notation for lists. The |(a : Set)| before the colon means the type of lists depends on a parameter, which is an arbitrary (small) type |a|. The underscore denotes where the argument is placed, so |[ a ]| is a list of elements from |a|, |[ ℕ ]| a list of natural numbers, etc. In the same way, |_∷_| is a function of two arguments, the first of type |a| (written in the place of the first underscore), the second of type |[ a ]| (written in place of the second underscore). The last line defines that the infix operator |_∷_| associates to the right (the |r|) and the |8| defines how tightly it binds (to determine whether an expression including other operators needs parentheses or not). In the remainder, we omit the |infix| declarations from this text, but use them to give operations the bindings we expect (so that, for example, multiplication binds tighter than addition). 
...
Next, we define some functions on |ℕ| and |[ ℕ ]|. In particular, to define the maximum of a list of natural numbers, we need to be able to find the maximum of a pair of natural numbers. We define this as follows:
\begin{code}
max : ℕ → ℕ → ℕ
max zero     n        = n
...
%
When needed (when Agda cannot infer them), implicit arguments can be provided in curly brackets. %We can define a length-function for |ℕ| by partial application:
