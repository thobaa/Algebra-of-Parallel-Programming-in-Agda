...
\end{spec}
Here we note that we placed the placement of |ℕ| in the first line. They are placed on the right side of the colon because they are indices of |_≤_|. This means that we are defining a type family (consisting of the types |m ≤ n| for every |m n : ℕ|. We can see that we need to do this from the fact that the two constructors produce elements of different types, |zero ≤ n| and |suc m ≤ suc n|, respectively. We also make note of the names we have given the constructors. In the remainder of this thesis, we will make use of the convention that |Pxy| (without the spaces) is an element of datatype |P x y| (with spaces), so |m≤n| is a proof of that |m ≤ n|.

If we have an element of type |m ≤ n|, it is either constructed by |z≤n|, which means that |m| is |zero|, so that the proposition $m \le n$ is true. Otherwise, it is constructed by |s≤s|, and we must have |m = suc m'|, |n = suc n'| for some |m'|, |n'| and an element of type |m' ≤ n'|. But then, the proposition $m' \le n'$ is true, and hence, again $m \le n$ is true. So providing an element of type |m ≤ n| means providing a proof that |m ≤ n|. Intuitively, we see that identifying propositions and types makes sense. 
...
The last predicate logic operation is negation. Constructively, the negation of a proposition means that the proposition implies falsity. To define this, we first define the empty type as a type with no constructors to represent falsity:
\begin{code}
data ⊥ : Set where
\end{code}
