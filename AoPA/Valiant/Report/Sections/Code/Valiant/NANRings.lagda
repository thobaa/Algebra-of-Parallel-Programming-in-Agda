...
\end{code}
%endif
\subsubsection{Proof that they are nonassociative semiings}
...
The approach for proving things about addition involves lifting statements into the ground nonassociative semiring. We examplify by proving the |identityˡ|-lemma for |Vec|, that is, that |zeroVec| is the left identity of |_v+_|:
\begin{code}
...
\end{code}
%endif
Then, we can define instances of |CommutativeMonoid| and |NonassociativeSemiring|:
\begin{code}
...
rearr = {!!}
\end{code}
Proving things about multiplication also involves moving the properties down to the ground nonassociative semiring, but here, the path is longer. We examplify the beginning of this path by giving the proof that |zeroTri| is a left zero of |_t*_|, and that |_t*_| distributes over |_t*_|, on the left:
%if False
\begin{code}
postulate 
