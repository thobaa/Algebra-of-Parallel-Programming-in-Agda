...
\end{code}
%endif
We begin to prove that they form a commutative monoid. This is done by giving an element of the type (we supply the implicit arguments to |_M≈_| make Agda understand what size of matrices we are interested in)
\begin{spec}
∀ {m n} → IsCommutativeMonoid (_M≈_ {m} {n}) _M+_ zeroMatrix
\end{spec}
We give this element the name |M+-isCommutativeMonoid|:
\begin{code}
M+-isCommutativeMonoid :  ∀ {m n} → IsCommutativeMonoid 
                          (_M≈_ {m} {n}) _M+_ zeroMatrix
\end{code}
To define this, we need to give a proof that it is a monoid, and a proof that it is commutative. Here, we only include the proof of the |comm|-axiom, the proofs involved in proving |isMonoid| are similar to it. The proof should be an element of type
\begin{code}
...
To prove that the square matrices form a nonassociative semiring, we need to give an element of type (which we name |M-isNonassociativeSemiring|)
\begin{spec}
∀ {n} → IsNonassociativeSemiring (_M≈_ {n}) _M+_ _M*_ zeroMatrix
\end{spec}
...
\end{code}
Agda tells us that the type of |M-zeroˡ| reduces to
\begin{spec}
...
zeroVector _ = R0
\end{code}
So we prove that |zeroVector ∙ v ≈ R0| for any |v| (this does not mean that |zeroVector| is a zero element anywhere, since the dot product |_∙_| is not a binary operation), by induction on the length using our lemma from Section \ref{e'e''e}:
\begin{code}
...
∙-zero {suc n} {v}  = e'∙e''≈e R+-CM  (proj₁ R-zero (head v)) 
                                      (∙-zero {n})
\end{code}
since Agda reduces the type to 
\begin{spec}
