...
_*_ {deeper s₁ s₂} {one} {deeper t₁ t₂} (CVec (two u₁ u₂)) (RVec (two v₁ v₂)) = quad ((cVec u₁) * (rVec v₁)) ((cVec u₁) * (rVec v₂)) ((cVec u₂) * (rVec v₁)) ((cVec u₂) * (rVec v₂))
_*_ {one} {deeper y y'} (RVec u) (CVec v) = Sing (u ∙ v)
_*_ {one} {deeper y y'} (RVec (two u v)) (quad A B C D) = RVec (two (unMat2 (rVec u * A  +  rVec v * C)) (unMat2 (rVec u * B  +  rVec v * D)))
_*_ {deeper r1 r2} {deeper y y'} (quad A B C D) (CVec (two e f)) = CVec (two (unMat1 (A * cVec e  +  B * cVec f)) (unMat1 (C * cVec e  +  D * cVec f)))
